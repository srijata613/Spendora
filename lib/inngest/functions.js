...
export const billReminder = inngest.createFunction(
  {
    id: "bill-reminder",
    name: "Bill Reminder",
    triggers: [{ cron: "0 9 * * *" }], // every day 9 AM
  },
  async ({ step }) => {

    const upcomingBills = await step.run("fetch-upcoming-bills", async () => {
      return db.transaction.findMany({
        where: {
          isRecurring: true,
          nextRecurringDate: {
    ...
    for (const bill of upcomingBills) {
      ...
      await sendEmail({
        ...
        react: (
          <EmailTemplate
            userName={bill.user.name || "User"}
            type="billReminder"
            data={{
              description: bill.description,
              amount: Number(bill.amount).toFixed(2),
              date: bill.nextRecurringDate,
    ...
    return { remindersSent: upcomingBills.length };
  }
);